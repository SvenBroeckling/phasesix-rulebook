\worldchapter{Rolls and Checks}{rolls}
\index{Rolls}\index{Checks}
\label{ch:rolls}

\begin{multicols}{2}

Whenever a character performs an action with an uncertain outcome, dice are rolled for a suitable value, which is usually determined by the game master. The outcome of the roll indicates whether and how well the action succeeded. This is called a \textit{roll} or \textit{check}.
...
A check is therefore a roll with a \textit{number} of dice, where a \textit{minimum roll} must be achieved, which can be modified by a \textit{level of difficulty}. The elements are explained in more detail below.
...
\section{The number of dice}
\index{Number of dice}\index{Dice}

The number of dice rolled for a check corresponds exactly to the Total Skill Value. This value is the sum of the skills's Base Attribute and any bonuses granted by Character Templates. 

\begin{pexample}[Example]
A character with a Total Value of 4 in 'Intimidate' (e.g., Apprehension 1 + Template 3) rolls 4 six-sided dice.
\end{pexample}
...
If the total value is 0 or negative, the character cannot make the check without further help, he is simply too bad at this skill. However, \textit{bonus dice} or \textit{destiny dice} can be used even if the value is negative.
...
\section{Levels of difficulty}
\index{Levels of difficulty}
...
A +3 test means that the minimum roll is increased by 3, so usually 8+. This is where the fact that all dice are rolled \textit{further} comes into play, i.e. they are \textit{exploding dice}.

The usual difficulty levels are:

\begin{itemize}
    \item \textbf{-2}: very easy
    \item \textbf{-1}: easy
    \item \textbf{0}: normal
    \item \textbf{+1}: difficult
    \item \textbf{+3}: very difficult
    \item \textbf{+6}: extremely difficult
    \item \textbf{+12}: impossible
\end{itemize}
...
\index{Exploding Dice}

In \trans{book_title}, it is possible for the minimum rolls to be higher than 6+, sometimes significantly higher. The \textit{exploding dice} rule applies to any roll.
...
\section{Critical successes}
\index{Critical successes}

Similar to \textbf{critical hits} in combat, other rolls can result in critical successes. If a die shows a result of at least 11, this is a critical success. This is the equivalent of rolling another \textit{exploding die}, which will again give a result of 5+. Changes to the character's \textit{minimum roll} are not applied here.
...
\begin{itemize}
    \item \textbf{Roll 5+}: normal success
    \item \textbf{Roll 11+}: critical success - results in one additional success
    \item \textbf{Roll 17+}: megacritical success - results in two additional successes
    \item \textbf{Roll 23+}: megacritical success - gives three additional successes
    \item \textbf{Roll 29+}: megacritical success - gives four additional successes
    \item etc.
\end{itemize}
...
The number of re-rolls a character has is a character trait that can be obtained through templates.
...
Destiny dice can be used as bonus dice as well as for re-rolling. A result of 4+ on a destiny die is \textbf{always} a success, regardless of the difficulty of the check. If the destiny die is used as a reroll, this applies to all dice rolled in that check.
...
Whenever the group as a whole needs to pass a check, group rolls are used. For example, instead of asking each player to make an Apprehension check, the GM can ask the whole group to make an Appreprehension check. If the check is successful, the effect applies to all characters in the group.
...
If a person is observing a character or the whole group, the item with the \textit{highest} concealment value is used for the concealment roll.

The person observing now rolls a number of dice equal to their Perception value \textit{plus} the determined Conceal.
\end{multicols}
